...
TileMatrixSet* buildTileMatrixSet(std::string fileName){
        ...
        crs = pElem->GetText();
        //FIXME: controle et normalisation du nom du CRS Ã  faire
        ...
        if (listTM.size()==0){
                LOGGER_ERROR("Aucun tileMatrix trouvÃ© dans le tileMatrixSet" << id <<" : il est invalide!!");
                return NULL;
        }
...
Pyramid * buildPyramid(std::string fileName, std::map<std::string, TileMatrixSet*> &tmsList){
        ...
        if (!pElem){
                LOGGER_ERROR(fileName << "La pyramide n'a pas de TMS. C'est un problÃ¨me.");
                return NULL;
        }
        ...
        for( pElem; pElem; pElem=pElem->NextSiblingElement( "level")){
                ...
                std::string baseDir;
                int channels;
                int32_t minTileRow=-1; // valeur conventionnelle pour indiquer que cette valeur n'est pas renseignÃ©e.
            ...
            pElemLvl = hLvl.FirstChild("blockHeight").Element();
                ...
                if (pElemTMSL){ // le bloc TMSLimits n'est pas obligatoire, mais s'il est lÃ , il doit y avoir tous les champs.
        ...
        if (levels.size()==0){
                LOGGER_ERROR("Aucun level n'a pu Ãªtre chargÃ© pour la pyramide "<< fileName);
                return NULL;
        }
...
Layer * buildLayer(std::string fileName, std::map<std::string, TileMatrixSet*> &tmsList){
        ...
        if (WMSCRSList.size()==0){
                LOGGER_WARN(fileName << ": Aucun CRS autorisÃ© pour le WMS");
        }
        ...
        for (pElem; pElem; pElem=pElem->NextSiblingElement("pyramid")){
                ...
                if (!pyramid){
                        LOGGER_ERROR("La pyramide " << pElem->GetText() << " ne peut Ãªtre chargÃ©e");
                        //FIXME: que faut-il faire des pyramides dÃ©jÃ  crÃ©Ã©es? Faut-il les detruire? ou vector s'en charge?
                        return NULL;
                }
                pyramids.push_back(pyramid);
        }
        if (pyramids.size()==0){
                // FIXME: pas forcÃ©ment critique si on a un cache d'une autre nature (jpeg2000 par exemple).
                LOGGER_ERROR("Aucune pyramide associÃ© au layer "<< fileName);
                return NULL;
        }
...
bool ConfLoader::buildTMSList(std::string tmsDir,std::map<std::string, TileMatrixSet*> &tmsList){
        ...
        // lister les fichier du rÃ©pertoire tmsDir
        std::vector<std::string> tmsFiles;
        std::string tmsFileName;
    struct dirent *fileEntry;
    DIR *dir;
    if ((dir = opendir(tmsDir.c_str())) == NULL){
        LOGGER_FATAL("Le rÃ©pertoire des TMS " << tmsDir << " n'est pas accessible.");
        return false;
    }
    ...
    if (tmsFiles.empty()){
        // FIXME:
        // Aucun TMS prÃ©sents. Ce n'est pas nÃ©cessairement grave si le serveur
        // ne sert pas pour le WMTS et qu'on exploite pas de cache tuilÃ©.
        // Cependant pour le moment (07/2010) on ne gÃ¨re que des caches tuilÃ©s
        LOGGER_FATAL("Aucun fichier *.tms dans le rÃ©pertoire " << tmsDir);
        return false;
    }

    // gÃ©nÃ©rer les TMS dÃ©crits par les fichiers.
    ...
    if (tmsList.size()==0){
        LOGGER_FATAL("Aucun TMS n'a pu Ãªtre chargÃ©!");
        return false;
    }
...
bool ConfLoader::buildLayersList(std::string layerDir,std::map<std::string, TileMatrixSet*> &tmsList, std::map<std::string,Layer*> &layers){
        ...
        // lister les fichier du rÃ©pertoire layerDir
        std::vector<std::string> layerFiles;
        std::string layerFileName;
    struct dirent *fileEntry;
    DIR *dir;
    if ((dir = opendir(layerDir.c_str())) == NULL){
        LOGGER_FATAL("Le rÃ©pertoire " << layerDir << " n'est pas accessible.");
        return false;
    }
    while ((fileEntry = readdir(dir))){
    ...
    if (layerFiles.empty()){
        LOGGER_FATAL("Aucun fichier *.lay dans le rÃ©pertoire " << layerDir);
        LOGGER_FATAL("Le serveur n'a aucune donnÃ©es Ã  servir. Domage...");
        return false;
    }

    // gÃ©nÃ©rer les Layers dÃ©crits par les fichiers.
    ...
    if (layers.size()==0){
        LOGGER_FATAL("Aucun layer n'a pu Ãªtre chargÃ©!");
        return false;
    }
