...
public:
    ...
    void AddPage ( cdxCDynamicPropPage *pPage ) {
        ASSERT ( pPage != NULL );
        AddPage ( *pPage );
    }
    void RemovePage ( cdxCDynamicPropPage *pPage ) {
        ASSERT ( pPage != NULL );
        RemovePage ( *pPage );
    }
...
protected:
    ...
    virtual void OnSetActive ( cdxCDynamicPropPage & rPage, BOOL bStatus ) {
        if ( IsWindow() && IsWizard() ) Layout();
    }
