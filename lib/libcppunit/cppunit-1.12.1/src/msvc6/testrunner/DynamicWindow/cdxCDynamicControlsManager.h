...
public:
    ...
    public:
        ...
        // validity check
        bool IsValid() const {
            return ( m_dX1pcnt <= m_dX2pcnt ) && ( m_dY1pcnt <= m_dY2pcnt );
        }
        ...
        // quick-use
        CRect operator() ( const CRect & rectOriginal, const CSize & szDelta ) const {
            return Transform ( rectOriginal,szDelta );
        }
    };
...
private:
    ...
    private:
        ...
        public:
            ...
            ControlEntry *GetNext() {
                return m_pNext;
            }
            CWnd & GetCWnd() {
                return m_rCtrl;
            }
            const ControlEntry *GetNext() const {
                return m_pNext;
            }
            const CWnd & GetCWnd() const {
                return m_rCtrl;
            }

            bool operator== ( const CWnd & ctrl ) const {
                return &m_rCtrl == &ctrl;
            }
        };
    ...
    public:
        ...
        virtual CRect GetOriginalPosition() const {
            return CRect ( m_rectOriginal );
        }
        virtual PositionSetup GetPositionSetup() const {
            return PositionSetup ( m_posSetup );
        }
        ...
        const ControlData *GetNext() const {
            return m_pNext;
        }
...
public:
    ...
    virtual ~cdxCDynamicControlsManager() {
        DoDestroyWindow();
    }
    ...
    bool IsReady() const {
        return ( m_pWnd != NULL ) && ::IsWindow ( m_pWnd->m_hWnd );
    }
    UINT GetTotalChildCnt() const {
        return m_iTotalCnt;
    }
    ...
    ControlPosition *AddSzXControl ( CWnd & ctrl, Mode modeX ) {
        return AddSzControl ( ctrl,modeX,mdNone );
    }
    ControlPosition *AddSzYControl ( CWnd & ctrl, Mode modeY ) {
        return AddSzControl ( ctrl,mdNone,modeY );
    }

    ControlPosition *AddSzControlEx ( CWnd & ctrl, BYTE dX1pcnt, BYTE dX2pcnt, BYTE dY1pcnt, BYTE dY2pcnt ) {
        return AddSzControlEx ( ctrl,PositionSetup ( dX1pcnt,dX2pcnt,dY1pcnt,dY2pcnt ) );
    }
    ControlPosition *AddSzXControlEx ( CWnd & ctrl, BYTE dX1pcnt, BYTE dX2pcnt ) {
        return AddSzControlEx ( ctrl,dX1pcnt,dX2pcnt,0,0 );
    }
    ControlPosition *AddSzYControlEx ( CWnd & ctrl, BYTE dY1pcnt, BYTE dY2pcnt ) {
        return AddSzControlEx ( ctrl,0,0,dY1pcnt,dY2pcnt );
    }
    ...
    //
    virtual bool Enable ( bool bForce = false ) {
        if ( !bForce ) {
            --m_iDisabledCnt;
            ASSERT ( m_iDisabledCnt >= 0 );
        } else m_iDisabledCnt = 0;
        return m_iDisabledCnt == 0;
    }
...
public:
    ...
    bool StretchWindow ( const CSize & szDelta ) {
        ASSERT ( IsReady() );
        return StretchWindow ( *m_pWnd,szDelta );
    }
    bool StretchWindow ( int iAddPcnt ) {
        ASSERT ( IsReady() );
        return StretchWindow ( *m_pWnd,iAddPcnt );
    }
    CSize GetWindowSize() {
        ASSERT ( IsReady() );
        return GetWindowSize ( *m_pWnd );
    }
    bool RestoreWindowPosition ( LPCTSTR lpszProfile, UINT restoreFlags = rflg_none ) {
        ASSERT ( IsReady() );
        return RestoreWindowPosition ( *m_pWnd,lpszProfile,restoreFlags );
    }
    bool StoreWindowPosition ( LPCTSTR lpszProfile ) {
        ASSERT ( IsReady() );
        return StoreWindowPosition ( *m_pWnd,lpszProfile );
    }
...
public:
    ...
    static CImageList & GetSizeIconImageList ( CSize * pSzBmp = NULL ) {
        if ( pSzBmp ) *pSzBmp = cdxCSizeIconCtrl::M_ilImage.Size();
        return cdxCSizeIconCtrl::M_ilImage;
    }
};
...
inline cdxCDynamicControlsManager::ControlPosition *cdxCDynamicControlsManager::AddSzControl ( CWnd & ctrl, Mode modeX, Mode modeY ) {
    ...
    switch ( modeX ) {
    default	:
        ASSERT ( false );			// unknown value for modeX
    case	mdNone	:
        break;
    case	mdRepos	:
        dX1pcnt	=	dX2pcnt	=	100;
    ...
    case	mdResize	:
        ...
        break;
    case	mdRelative:
        dX1pcnt	=	dX2pcnt	=	100 / 2;
        break;
    }

    switch ( modeY ) {
    default	:
        ASSERT ( false );			// unknown value for modeY
    case	mdNone	:
        break;
    case	mdRepos	:
        dY1pcnt	=	dY2pcnt	=	100;
