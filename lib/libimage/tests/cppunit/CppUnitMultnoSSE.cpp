...
class CppUnitMultnoSSE : public CPPUNIT_NS::TestFixture
{
  CPPUNIT_TEST_SUITE( CppUnitMultnoSSE );
...
protected:
  ...
  void performance() {
    ...
    cerr << " -= Multiplications no SSE =-" << endl;
    ...
    cerr << " -= Addition-Multiplications no SSE =-" << endl;
    for(int d1 = 0; d1 < 4; d1++)
      for(int d2 = 0; d2 < 4; d2++) {
...
//    cerr << "Test add_mult no SSE OK" << endl;
  }
